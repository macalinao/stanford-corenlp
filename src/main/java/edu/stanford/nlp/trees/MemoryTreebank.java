...
/**
 * A
 * <code>MemoryTreebank</code> object stores a corpus of examples with given
 * tree structures in memory (as a
 * <code>List</code>).
 *
...
public final class MemoryTreebank extends Treebank implements FileProcessor, List<Tree> {
    ...
    /**
     * THIS IS AT PRESENT DELETED, UNLESS PROBLEMS RECUR. If this is true, the
     * system will retry opening files a few times before concluding that there
     * is really a problem. This seems to be necessary with NFS on Linux boxes
     * -- at least the DB ones.
     */
    ...
    /**
     * Create a new tree bank. The trees are made with a
     * <code>LabeledScoredTreeReaderFactory</code>.
     * <p/>
     * <i>Compatibility note: Until Sep 2004, this used to create a Treebank
     * with a SimpleTreeReaderFactory, but this was changed as the old default
     * wasn't very useful, especially to naive users. This one now uses a
     * LabledScoredTreeReaderFactory with a no-op TreeNormalizer.</i>
     */
    ...
    /**
     * Create a new tree bank, using a specific TreeNormalizer. The trees are
     * made with a
     * <code>LabeledScoredTreeReaderFactory</code>.
     * <p/>
     * <i>Compatibility note: Until Sep 2004, this used to create a Treebank
     * with a SimpleTreeReaderFactory, but this was changed as the old default
     * wasn't very useful, especially to naive users.</i>
     */
    ...
    /**
     * Create a new tree bank. The list of trees passed in is simply placed in
     * the Treebank. It is not copied.
     *
     ...
     *
     * @param initialCapacity The initial size of the underlying Collection, (if
     * a Collection-based storage mechanism is being provided)
     */
    ...
    /**
     * Load a collection of parse trees from the file of given name. Each tree
     * may optionally be encased in parens to allow for Penn Treebank style
     * trees. This methods implements the
     * <code>FileProcessor</code> interface.
     *
...
//                    t1 = t1.parent(t);
                                    ...
                                    Map<Integer, String> roleMap = (Map<Integer, String>) ((CoreLabel) t1.label()).get(CoreAnnotations.CoNLLSRLAnnotation.class);
                                    if (roleMap == null) {
...
//              parseTrees.add(t);
    ...
    /**
     * Load a collection of parse trees from a Reader. Each tree may optionally
     * be encased in parens to allow for Penn Treebank style trees.
     *
     * @param r The reader to read trees from. (If you want it buffered, you
     * should already have buffered it!)
     */
     ...
     * @param r The reader to read trees from. (If you want it buffered, you
     * should already have buffered it!)
     * @param id An ID for where these files come from (arbitrary, but something
     * like a filename. Can be
    ...
    /**
     * Get a tree by index from the Treebank. This operation isn't in the
     * <code>Treebank</code> feature set, and so is only available with a
     * <code>MemoryTreebank</code>, but is useful in allowing the latter to be
     * used as a
     * <code>List</code>.
     *
    ...
    /**
     * Returns the size of the Treebank. Provides a more efficient
     * implementation than the one for a generic
     * <code>Treebank</code>
     *
    ...
    /**
     * Return a MemoryTreebank where each Tree in the current treebank has been
     * transformed using the TreeTransformer. This Treebank is unchanged
     * (assuming that the TreeTransformer correctly doesn't change input Trees).
     *
    ...
    /**
     * Loads treebank grammar from first argument and prints it. Just a
     * demonstration of functionality. <br>
