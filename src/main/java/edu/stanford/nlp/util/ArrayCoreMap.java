...
/**
 * <p> Base implementation of {@link CoreMap} backed by Java Arrays. </p>
 *
 * <p> Reasonable care has been put into ensuring that this class is both fast
 * and has a light memory footprint. </p>
 *
 * <p> Note that like the base classes in the Collections API, this
 * implementation is <em>not thread-safe</em>. For speed reasons, these methods
 * are not synchronized. A synchronized wrapper could be developed by anyone so
 * inclined. </p>
 *
 * <p> Equality is defined over the complete set of keys and values currently
 * stored in the map. Because this class is mutable, it should not be used as a
 * key in a HashMap. </p>
 *
 * @author dramage
 * @author rafferty
...
public class ArrayCoreMap implements CoreMap, Serializable {
    ...
    /**
     * Initializes this ArrayCoreMap, pre-allocating arrays to hold up to
     * capacity key,value pairs. This array will grow if necessary.
     *
     ...
     */
    @SuppressWarnings("unchecked")
    public <VALUE, KEY extends Key<CoreMap, VALUE>> VALUE get(Class<KEY> key) {
        for (int i = size; i > 0;) {
            if (keys[--i] == key) {
     ...
     * {@inheritDoc}
     */
    public <VALUE, KEY extends Key<CoreMap, VALUE>> boolean has(Class<KEY> key) {
     ...
     */
    @SuppressWarnings("unchecked")
    public <VALUEBASE, VALUE extends VALUEBASE, KEY extends Key<CoreMap, VALUEBASE>> VALUE set(Class<KEY> key, VALUE value) {
     ...
     */
    @SuppressWarnings("unchecked")
    public <VALUE, KEY extends Key<CoreMap, VALUE>> VALUE remove(Class<KEY> key) {
     ...
     * {@inheritDoc}
     */
    public <VALUE, KEY extends Key<CoreMap, VALUE>> boolean containsKey(Class<KEY> key) {
        for (int i = 0; i < size; i++) {
            if (keys[i] == key) {
                return true;
            }
     ...
     * stored in the map. Because they may change over time, this class is not
     * appropriate for use as map keys.
     */
    @Override
    public int hashCode() {
        int keyscode = 0;
    ...
    public void prettyLog(RedwoodChannels channels, String description) {
        ...
        List<Class> keys = new LinkedList<Class>();
        for (Class<?> key : this.keySet()) {
            keys.add(key);
        }
